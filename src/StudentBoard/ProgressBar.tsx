...
const ProgressBar = () => {
  const student_score = useStore(
    (state) => state.your_best_submission?.points || 0
  );
  ...
  console.log('PROGRESS BAR', your_level_details);
  ...
  const progress = lower_is_better
    ? maxScore - student_score
    : student_score - minScore;
  ...
  return (
    <ShadowedContainer>
      {/* <BootstrapModal show={true} handleClose={() => {}} /> */}
          ...
          {your_level_details?.level == 6 ? (
            ...
            <>
              <span
                ...
                <strong>Next level at: </strong>

                {lower_is_better
                  ? Number(your_range_details.lower_limit) - 1
                  : Number(your_range_details.upper_limit) + 1 || 0}
                {/* - {lower_is_better ? your_range_details.lower_limit : your_range_details.upper_limit || 0} */}
              </span>
              <span>
                <>
                  {' '}
                  You need <strong>{progressToNextLevel + 1} </strong>
...
{
  /* create a bootstrap modal component that shows the levels that have been passed
   */
}
...
//   handleClose: () => void;
// }
// const BootstrapModal = ({ show, handleClose }: ModalProps) => {
//   return (
//     <div
